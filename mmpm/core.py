...
from mmpm.objects import MagicMirrorModule
            ...
            if column_number > 0:
                ...
                for idx, _ in enumerate(td_soup):
                    if idx == 0:
                        ...
                        repo = str(repo)
                        title = utils.sanitize_name(str(title))
...
#def __display_modules__(modules: List[List[str]], rows: int, columns: int):
#    libmmpm = cdll.LoadLibrary('./libmmpm.so')
#    _display_modules = libmmpm.display_modules
#    _display_modules.argtypes = [POINTER(POINTER(c_char_p)), c_int, c_int]
#    #_display_modules( rows, columns)
